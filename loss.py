...
def alpha_estimation_loss(pred_alpha, gt_alpha, pred_trimap_argmax):
    ...
    return torch.abs(masked_pred_alpha - masked_gt_alpha).sum() / (num_unknown_pixel + 1e-8)
...
def task_uncertainty_loss(pred_trimap, pred_trimap_argmax, pred_alpha, gt_trimap, gt_alpha, log_sigma_t_sqr, log_sigma_a_sqr):
    ...
    overall = 3e2 * Lt / (2 * torch.exp(log_sigma_t_sqr)) + 1e2 * La / torch.exp(log_sigma_a_sqr) + (log_sigma_t_sqr + log_sigma_a_sqr) / 2
